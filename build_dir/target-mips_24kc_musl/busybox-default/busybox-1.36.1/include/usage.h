...
#define tar_trivial_usage \
	...
	"vokO] " \
	"[-f TARFILE] [-C DIR] " \
	IF_FEATURE_TAR_FROM("[-T FILE] [-X FILE] "IF_FEATURE_TAR_LONG_OPTIONS("[LONGOPT]... ")) \
	"[FILE]..." \
...
#define env_trivial_usage \
       "[-i0] [-u NAME]... [-] [NAME=VALUE]... [PROG ARGS]" \
...
#define touch_trivial_usage \
       ...
       IF_FEATURE_TOUCH_SUSV3(" [-d DATE] [-t DATE] [-r FILE]") \
       " FILE..." \
...
#define chattr_trivial_usage \
       "[-R] [-v VERSION] [-p PROJID] [-+=AacDdijsStTu] FILE..." \
...
#define cryptpw_trivial_usage \
       "[-P FD] [-m TYPE] [-S SALT] [PASSWORD] [SALT]" \
...
#define su_trivial_usage \
       "[-lmp] [-s SH] [-] [USER [FILE ARGS | -c 'CMD' [ARG0 ARGS]]]" \
...
#define ascii_trivial_usage NOUSAGE_STR \

#define ascii_full_usage "" \
...
#define microcom_trivial_usage \
       "[-d DELAY_MS] [-t TIMEOUT_MS ] [-s SPEED] [-X] TTY" \
...
#define ftpd_trivial_usage \
       "[-wvS]"IF_FEATURE_FTPD_AUTHENTICATION(" [-a USER]")" [-t SEC] [-T SEC] [DIR]" \
...
#define taskset_trivial_usage \
       "[-ap] [HEXMASK"IF_FEATURE_TASKSET_CPULIST(" | -c LIST")"] { PID | PROG ARGS }" \
...
#define udhcpc6_trivial_usage \
       "[-fbq"IF_UDHCP_VERBOSE("v")"R] [-t N] [-T SEC] [-A SEC|-n] [-i IFACE] [-s PROG]\n" \
       "	[-p PIDFILE]"IF_FEATURE_UDHCP_PORT(" [-P PORT]")" [-ldo] [-r IPv6] [-x OPT:VAL]... [-O OPT]..." \
